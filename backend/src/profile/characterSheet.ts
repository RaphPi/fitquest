...
function smoothPath(pts: { x: number; y: number }[], yTop: number, yBot: number): string {
  ...
  for (let i = 0; i < pts.length - 1; i++) {
    ...
    const p2 = pts[i + 1];
    const p3 = pts[i + 2] ?? p2;
    // Bornes du segment en X : empêche tout débordement/boucle horizontale
    // (cas d'un dernier relevé rapproché dans le temps mais éloigné en valeur).
    const clampX = (x: number) => Math.max(p1.x, Math.min(p2.x, x));
